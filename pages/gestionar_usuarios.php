...
<div class="row w-100 mt-2 mb-2 p-1 fade-in">
    <div class="col-12 mt-1">
        <div class="filter-header mb-1">
            ...
            <button class="btn add-product-btn" data-bs-toggle="modal" data-bs-target="#usuarioModal" onclick="window.abrirModalUsuario()">
        ...
        <div class="table-container p-3 mb-2 shadow bg-white rounded">
            <div class="search-container mb-3 position-relative">
                ...
                <input type="text" id="search-usuario" class="form-control rounded-pill ps-5" placeholder="Buscar por nombre o rol...">
            </div>
                    ...
                    <?php if ($result && $result->num_rows > 0): ?>
                        <?php while($row = $result->fetch_assoc()): ?>
                            <tr data-id="<?= $row['ID'] ?>" 
                                ...
                                data-rol="<?= $row['rol_id'] ?>" 
                                data-distribuidor="<?= $row['distribuidor_codigo'] ?>">
                                <td><span class="badge bg-secondary"><?= $row['ID'] ?></span></td>
                                <td class="fw-bold text-dark"><?= htmlspecialchars($row['nombre']) ?></td>
                                <td><span class="badge bg-primary"><?= ucfirst(htmlspecialchars($row['rol_nombre'])) ?></span></td>
                                <td>
...
<div class="modal fade" id="usuarioModal" tabindex="-1" aria-labelledby="usuarioModalLabel" aria-hidden="true" data-bs-backdrop="static">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content border-0 shadow-lg" style="border-radius: 12px;">
            <div class="modal-header text-white" style="background: linear-gradient(135deg, #FF6B6B 0%, #D32F2F 100%); border-top-left-radius: 12px; border-top-right-radius: 12px;">
                <h5 class="modal-title fw-bold" id="usuarioModalLabel"><i class="bi bi-person-badge me-2"></i><span id="modal-title-text">Añadir Usuario</span></h5>
                ...
                <form id="form-usuario">
                    ...
                    <div class="mb-3">
                        ...
                        <div class="input-group">
                            ...
                            <input type="text" class="form-control mb-0" id="usr_nombre" name="nombre" placeholder="MMAYUSCULAS_SIN_ACENTOS" required>
                        </div>
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <div class="input-group">
                            ...
                            <input type="text" class="form-control mb-0" id="usr_password" name="password" placeholder="Clave de acceso">
                        </div>
                        <div class="form-text" id="usr_password_help">Dejar en blanco al editar si no deseas cambiarla.</div>
                    </div>
                    ...
                    <div class="mb-3">
                        <label for="usr_distribuidor" class="form-label fw-bold text-secondary">Código Distribuidor Vinculado (Opcional)</label>
                        <div class="input-group">
                            <span class="input-group-text bg-white"><i class="bi bi-building"></i></span>
                            <input type="number" class="form-control mb-0" id="usr_distribuidor" name="distribuidor_codigo" placeholder="Ej. 4063">
                        </div>
            ...
            <div class="modal-footer bg-light border-top-0" style="border-bottom-left-radius: 12px; border-bottom-right-radius: 12px;">
                ...
                <button type="button" class="btn btn-danger px-4 rounded-pill shadow-sm" id="btnGuardarUsuario">Guardar Usuario</button>
            </div>
        </div>
    </div>
</div>
