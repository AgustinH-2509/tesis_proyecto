...
export function initControlDevoluciones() {
                            ...
                            aceptados.forEach((aceptado, index) => {
                                if (aceptado.trim()) {
                                    ...
                                    aceptadoRow.innerHTML = `
                                        ...
                                        <td class="py-2" style="background-color: #f0fff4;">
                                            <strong class="text-success">Producto aceptado</strong>
                                        </td>
                                        <td class="py-2" style="background-color: #f0fff4;">
                                            ${observacion && observacion !== 'N/A' ? `<small class="text-muted">📝 ${observacion}</small>` : '<small class="text-muted">Sin observaciones</small>'}
                                        </td>
    ...
    modalProductsTable.addEventListener('click', async function(event) {
        if (event.target.classList.contains('reject-product-btn')) {
            ...
            // Verificar si hay rechazos existentes para este producto
            let hasRejections = false;
            const allRows = document.querySelectorAll('tr');
            for (let checkRow of allRows) {
                if (checkRow.innerHTML.includes('RECHAZADO') && checkRow.innerHTML.includes(`data-detalle-id="${detalleId}"`)) {
                    hasRejections = true;
                    break;
                }
            }
            
            const modalConfig = hasRejections ? {
                type: 'danger',
                icon: '⚠️',
                title: 'Aceptar Producto',
                message: '¡ATENCIÓN! Al aceptar este producto se eliminarán TODOS los registros de rechazo existentes.',
                details: 'Esta acción no se puede deshacer. Todos los motivos y observaciones de rechazo se perderán permanentemente.',
                confirmText: 'Sí, Aceptar de todos modos',
                confirmIcon: '<i class="bi bi-check-circle"></i>',
                cancelText: 'No, Conservar rechazos'
            } : {
                type: 'success',
                icon: '✅',
                title: 'Aceptar Producto',
                message: '¿Confirma que desea aceptar este producto?',
                details: 'El producto será marcado como aceptado en la devolución.',
            ...
            event.target.innerHTML = '<i class="bi bi-hourglass-split"></i> Procesando...';
            
            try {
                const success = await updateProductStatus(detalleId, 0); // 0 = aceptado
                if (success) {
                    showNotification(hasRejections ? 'Producto aceptado y registros de rechazo eliminados' : 'Producto aceptado exitosamente', 'success');
                } else {
                    // Restaurar botón en caso de error
                    event.target.innerHTML = '<i class="bi bi-check-circle"></i> Aceptar';
                    event.target.disabled = false;
                }
            } catch (error) {
                console.error('Error al aceptar producto:', error);
                showNotification('Error al procesar la aceptación', 'error');
                // Restaurar botón en caso de error
                event.target.innerHTML = '<i class="bi bi-check-circle"></i> Aceptar';
                event.target.disabled = false;
            }
    ...
    modalProductsTable.addEventListener('click', async function(event) {
        if (event.target.classList.contains('save-reject-btn') || event.target.classList.contains('add-another-reject-btn')) {
            console.log('🔍 Botón presionado:', event.target.classList.contains('add-another-reject-btn') ? 'Otro' : 'Guardar');
            
            const detalleId = event.target.getAttribute('data-detalle-id');
            const uniqueId = event.target.getAttribute('data-unique-id');
            
            console.log('🔍 DetalleId:', detalleId, 'UniqueId:', uniqueId);
            ...
            if (!cantidad || cantidad <= 0) {
                showNotification('Debe especificar una cantidad válida para rechazar', 'error');
                cantidadInput.focus();
                return;
            }
            ...
            // Obtener texto del motivo seleccionado para el modal
            const motivoTexto = motivoSelect.options[motivoSelect.selectedIndex].text;
            const isOtroButton = event.target.classList.contains('add-another-reject-btn');
            
            const modalConfig = {
                type: 'danger',
                icon: '❌',
                title: isOtroButton ? 'Guardar y Continuar Rechazando' : 'Guardar Rechazo',
                message: `¿Confirma que desea rechazar ${cantidad} unidad${cantidad > 1 ? 'es' : ''} de este producto?`,
                details: `<strong>Motivo:</strong> ${motivoTexto}<br>${observacion ? `<strong>Observación:</strong> ${observacion}` : '<em>Sin observaciones</em>'}${isOtroButton ? '<br><br>📋 <strong>Después de guardar se creará un nuevo formulario para rechazar cantidad adicional.</strong>' : ''}`,
                confirmText: isOtroButton ? 'Sí, Guardar y Continuar' : 'Sí, Guardar Rechazo',
                confirmIcon: '<i class="bi bi-x-circle"></i>',
                cancelText: 'Cancelar'
            };
            ...
            try {
                const success = await updateProductStatus(detalleId, 1, motivoId, observacion, cantidad, uniqueId);
                
                if (success) {
                    if (event.target.classList.contains('add-another-reject-btn')) {
                        ...
                        showNotification('Rechazo guardado. Verificando cantidad disponible...', 'success');
                        ...
                        setTimeout(async () => {
                            ...
                            console.log('🔍 Filas encontradas:', updatedRows.length);
                            
                            for (let row of updatedRows) {
                                const rowId = row.getAttribute('data-detalle-id');
                                console.log('🔍 Comparando:', rowId, 'con', detalleId);
                                if (rowId === detalleId.toString()) {
                                    targetRow = row.closest('tr');
                                    console.log('🔍 Fila encontrada!', targetRow);
                                    break;
                                }
                            }
                            
                            if (targetRow) {
                                // Buscar la celda que muestra la cantidad disponible
                                const cantidadCell = targetRow.querySelector('td:nth-child(2)');
                                if (cantidadCell) {
                                    // Extraer cantidad disponible del texto (formato: "X (original Y)")
                                    const cantidadText = cantidadCell.textContent.trim();
                                    console.log('🔍 Texto de cantidad:', cantidadText);
                                    ...
                                    if (cantidadDisponible > 0) {
                                        // Crear directamente el formulario con la cantidad disponible correcta
                                        console.log('🔍 Creando formulario directamente con cantidad disponible:', cantidadDisponible);
                                        
                                        // Crear formulario de rechazo con la cantidad disponible correcta
                                        const newRow = document.createElement('tr');
                                        newRow.classList.add('motivo-rechazo-row');
                                        const uniqueId = `${detalleId}-${Date.now()}`;
                                        newRow.innerHTML = `
                                        ...
                                        showNotification(`Nuevo formulario creado. Disponible: ${cantidadDisponible} unidades`, 'info');
                                    } else {
                                        showNotification('No hay más cantidad disponible para rechazar en este producto', 'warning');
                                    }
                                } else {
                                    console.log('🔍 No se encontró la celda de cantidad');
                                    showNotification('No se pudo verificar la cantidad disponible', 'warning');
                                }
                            } else {
                                console.log('🔍 No se encontró la fila objetivo');
                        ...
                        await fetchDetalleDevolucion(currentDevolucionId);
                        showNotification('Rechazo guardado exitosamente', 'success');
                    }
                } else {
                    // Restaurar botón en caso de error
                    if (event.target.classList.contains('add-another-reject-btn')) {
                ...
                console.error('Error en el manejo de rechazo:', error);
        ...
        } else if (event.target.classList.contains('remove-reject-btn')) {
            const uniqueId = event.target.getAttribute('data-unique-id');
            const row = document.querySelector(`#cantidad-${uniqueId}`).closest('tr');
            row.remove();
            showNotification('Formulario de rechazo cancelado', 'info');
